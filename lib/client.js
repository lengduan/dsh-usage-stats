...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const EN = {
      ...
      "远程访问需经远程门控通道（未安装远程访问插件或设备未配对）": "Remote access needs the remote gate channel (remote access plugin missing, or device not paired)",
      "用量接口不可用": "Usage API unavailable",
    };
    let LANG = (() => {
    ...
    // ── 取数 ──────────────────────────────────────────────────────────────
    // 本机直连打原路径；host 半的 loopback 围栏只对非 loopback 来源回 403，
    // 那时（远程访问）改走 remote-web-ui 的 /remote 门控通道：配对后由宿主
    // 以 127.0.0.1 重发，围栏放行。首次探测成功后记住，不再多一次 403 往返；
    // 没装该插件时这里继续失败，只影响远程场景，本机使用不受影响。
    const LOCAL_PATH = "/usage/api";
    const REMOTE_PATH = "/remote/usage/api";
    ...
    function api(payload) {
      ...
      return post(viaRemoteChannel ? REMOTE_PATH : LOCAL_PATH)
        .then((r) => {
          if (r.status !== 403 || viaRemoteChannel) return r;
          viaRemoteChannel = true;
          return post(REMOTE_PATH);
        })
        .then(async (r) => ({ status: r.status, res: await r.json().catch(() => null) }))
        .then(({ status, res }) => {
          // 门控通道不存在（未装远程访问插件）或设备未配对时，响应体不是本插件的形状。
          if (!res || typeof res.ok !== "boolean") {
            throw new Error(status === 403 || status === 404
              ? t("远程访问需经远程门控通道（未安装远程访问插件或设备未配对）")
              : t("用量接口不可用"));
          }
          return res;
        });
