...
import { isLoopbackRequest } from './loopback-fence.js'
    ...
    if (webServer && typeof webServer.register === 'function') {
      ctx.effect(() => webServer.register({
        ...
        handler: async (req, res) => {
          // 绑定到非 loopback 时本路由不在宿主改写白名单里，必须自校验来源。
          // 远程访问改走 remote-web-ui 的 /remote 门控通道，配对后由宿主以
          // 127.0.0.1 重发，届时这里的 socket 与 Host 都是 loopback。
          if (!isLoopbackRequest(req)) {
            sendJson(res, { ok: false, error: 'forbidden: loopback-only' }, 403)
            return
          }
          try {
